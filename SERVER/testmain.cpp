...
int main() {
    ...
    cout << "当前数据库: " << currentDbName << endl;

    // === 测试表准备 ===
    process_sql("DROP TABLE students;", db, parser, currentDbName);
    process_sql("CREATE TABLE students (id INT PRIMARY KEY, name CHAR(50), age INT, score INT, graduated BOOL);",
        db, parser, currentDbName);

    // 插入测试数据
    process_sql("INSERT INTO students VALUES (1, 'Alice', 20, 85, true);", db, parser, currentDbName);
    process_sql("INSERT INTO students VALUES (2, 'Bob', NULL, 92, false);", db, parser, currentDbName);
    process_sql("INSERT INTO students VALUES (3, 'Charlie', 22, NULL, true);", db, parser, currentDbName);
    process_sql("INSERT INTO students VALUES (4, 'David', 20, 78, NULL);", db, parser, currentDbName);
    process_sql("INSERT INTO students VALUES (5, 'Eve', NULL, NULL, NULL);", db, parser, currentDbName);

    // === 新增 UPDATE 测试用例 ===
    cout << "\n=== UPDATE 语句测试（比较运算符）===" << endl;

    // 1. 测试 > 运算符
    process_sql("UPDATE students SET name = '优秀生' WHERE score > 85;", db, parser, currentDbName);
    process_sql("SELECT * FROM students;", db, parser, currentDbName); // 查看更新结果

    // 2. 测试 >= 运算符
    process_sql("UPDATE students SET name = '达标生' WHERE score >= 80;", db, parser, currentDbName);
    process_sql("SELECT id, name, score FROM students;", db, parser, currentDbName);

    // 3. 测试 < 运算符
    process_sql("UPDATE students SET name = '需努力' WHERE score < 80;", db, parser, currentDbName);
    process_sql("SELECT id, name, score FROM students WHERE score IS NOT NULL;", db, parser, currentDbName);

    // 4. 测试 <= 运算符
    process_sql("UPDATE students SET graduated = true WHERE age <= 20;", db, parser, currentDbName);
    process_sql("SELECT id, name, age, graduated FROM students;", db, parser, currentDbName);

    // 5. 测试 = 运算符
    process_sql("UPDATE students SET score = 100 WHERE name = 'Alice';", db, parser, currentDbName);
    process_sql("SELECT name, score FROM students WHERE name = 'Alice';", db, parser, currentDbName);

    // 6. 测试 <> 运算符
    process_sql("UPDATE students SET score = score + 5 WHERE id <> 3;", db, parser, currentDbName);
    process_sql("SELECT id, name, score FROM students;", db, parser, currentDbName);

    // 7. 测试 NULL 值的更新
    process_sql("UPDATE students SET age = 18 WHERE age IS NULL;", db, parser, currentDbName);
    process_sql("SELECT id, name, age FROM students;", db, parser, currentDbName);

    // 8. 测试反向条件更新
    process_sql("UPDATE students SET name = '毕业生' WHERE graduated <> false;", db, parser, currentDbName);
    process_sql("SELECT name, graduated FROM students;", db, parser, currentDbName);

    // 9. 测试边界值更新
    process_sql("UPDATE students SET score = 60 WHERE score <= 60;", db, parser, currentDbName);
    process_sql("SELECT id, score FROM students ORDER BY score;", db, parser, currentDbName);

    // 10. 测试多条件更新
    process_sql("UPDATE students SET name = '特殊生' WHERE age > 20 AND score IS NOT NULL;", db, parser, currentDbName);
    process_sql("SELECT id, name, age, score FROM students;", db, parser, currentDbName);
